...
public class AttachmentUtils {
    ...
    public static int cleanupOrphanedFiles(Context ctx, List<Note> allNotes) {
        ...
        // На случай, если раньше использовались локальные thumb-файлы — убираем старую папку.
        File legacyThumbsDir = new File(ctx.getFilesDir(), "attachment_thumbs");
        File[] legacyThumbs = legacyThumbsDir.listFiles();
        if (legacyThumbs != null) {
            for (File thumb : legacyThumbs) {
                if (thumb != null) thumb.delete();
            }
        }
        if (legacyThumbsDir.exists()) legacyThumbsDir.delete();

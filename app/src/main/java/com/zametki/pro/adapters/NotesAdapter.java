...
public class NotesAdapter extends RecyclerView.Adapter<NotesAdapter.VH> {
    ...
    public void onBindViewHolder(@NonNull VH h, int pos) {
        ...
        h.itemView.setBackgroundResource(n.isPinned()
                ? R.drawable.bg_card_pinned_fill : R.drawable.bg_card_note_fill);
        if (h.cardBorder != null) {
            h.cardBorder.setBackgroundResource(n.isPinned()
    ...
    private void renderAttachmentsPreview(LinearLayout container, Note n) {
        ...
        View item = createAttachmentPreviewCell(atts.get(0));
        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                dp(viewMode == PrefsManager.VIEW_GRID ? 110 : 140));
        lp.bottomMargin = dp(6);
        item.setLayoutParams(lp);
        container.addView(item);
    }

    private View createAttachmentPreviewCell(Attachment attachment) {
        ...
        thumb.setLayoutParams(new android.widget.FrameLayout.LayoutParams(
        ...
        badge.setBackgroundResource(R.drawable.bg_badge_gold);
        badge.setPadding(dp(6), dp(2), dp(6), dp(2));
        badge.setTextColor(ContextCompat.getColor(ctx, R.color.text_on_gold));
        badge.setTextSize(9f);
        badge.setText(typeLabelForAttachment(attachment));
        frame.addView(badge);

        ImageView playOverlay = new ImageView(ctx);
        android.widget.FrameLayout.LayoutParams playLp = new android.widget.FrameLayout.LayoutParams(dp(34), dp(34));
        playLp.gravity = android.view.Gravity.CENTER;
        playOverlay.setLayoutParams(playLp);
        playOverlay.setImageResource(R.drawable.ic_play);
        playOverlay.setPadding(dp(7), dp(7), dp(7), dp(7));
        playOverlay.setBackgroundResource(R.drawable.bg_play_overlay);
        playOverlay.setColorFilter(Color.WHITE);
        playOverlay.setVisibility(View.GONE);
        frame.addView(playOverlay);

        bindAttachmentPreview(thumb, playOverlay, attachment);
        return frame;
    }
    ...
    private String typeLabelForAttachment(Attachment attachment) {
        ...
        switch (attachment.type) {
            ...
            default:
                if (attachment.fileName != null) {
                    int dot = attachment.fileName.lastIndexOf('.');
                    if (dot > 0 && dot < attachment.fileName.length() - 1) {
                        String ext = attachment.fileName.substring(dot + 1).toUpperCase();
    ...
    static class VH extends RecyclerView.ViewHolder {
        ...
        View categoryDot, colorBar, selectionOverlay, swipeLayer, swipeBg, contentLayer, cardBorder;
        ...
        VH(@NonNull View v) {
            ...
            cardBorder = v.findViewById(R.id.v_card_border);

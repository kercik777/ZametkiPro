...
public class AttachmentsEditorAdapter extends RecyclerView.Adapter<AttachmentsEditorAdapter.VH> {
    ...
    public AttachmentsEditorAdapter(Context ctx, List<Attachment> data, Callback cb) {
        ...
        this.data = data;
        this.cb = cb;
        this.thumbCache = new LruCache<>(100);
    }
    ...
    public void onBindViewHolder(@NonNull VH h, int pos) {
        ...
        switch (a.type) {
            case Attachment.TYPE_IMAGE:
                h.thumbIcon.setImageResource(R.drawable.ic_image);
                loadImageThumbAsync(h, a);
                break;
            case Attachment.TYPE_VIDEO:
                h.thumbIcon.setImageResource(R.drawable.ic_videocam);
                h.playOverlay.setVisibility(View.VISIBLE);
                h.typeBadge.setVisibility(View.VISIBLE);
                h.typeBadge.setText("VIDEO");
                loadVideoThumbAsync(h, a);
                break;
            case Attachment.TYPE_AUDIO:
                h.thumbIcon.setImageResource(R.drawable.ic_audio);
                h.typeBadge.setVisibility(View.VISIBLE);
    ...
    private void loadImageThumbAsync(VH h, Attachment a) {
        if (a.fileName == null || a.fileName.isEmpty()) return;
        String key = "img:" + a.fileName + ":" + columns;
        h.thumb.setTag(key);
        ...
        AppExecutors.getInstance().diskIO().execute(() -> {
            Bitmap bmp = decodeImageThumb(a.fileName);
            if (bmp == null) return;
            thumbCache.put(key, bmp);
            AppExecutors.getInstance().mainThread(() -> {
                Object tag = h.thumb.getTag();
    ...
    private Bitmap decodeImageThumb(String fileName) {
        File f = AttachmentUtils.getFile(ctx, fileName);
        ...
        try {
            ...
            int sample = 1;
            int target = columns >= 4 ? 150 : (columns >= 2 ? 250 : 600);
    ...
    private void loadVideoThumbAsync(VH h, Attachment a) {
        if (a.fileName == null || a.fileName.isEmpty()) return;
        String key = "vid:" + a.fileName;
        h.thumb.setTag(key);
        Bitmap cached = thumbCache.get(key);
        if (cached != null && !cached.isRecycled()) {
            h.thumb.setImageBitmap(cached);
        ...
        AppExecutors.getInstance().diskIO().execute(() -> {
            Bitmap bmp = decodeVideoThumb(a.fileName);
            if (bmp == null) return;
            thumbCache.put(key, bmp);
            AppExecutors.getInstance().mainThread(() -> {
                Object tag = h.thumb.getTag();
    ...
    private Bitmap decodeVideoThumb(String fileName) {
        File f = AttachmentUtils.getFile(ctx, fileName);
        if (!f.exists()) return null;
        try {
            MediaMetadataRetriever mmr = new MediaMetadataRetriever();
            mmr.setDataSource(f.getAbsolutePath());
            Bitmap bmp = mmr.getFrameAtTime(1_000_000);
            mmr.release();
            return bmp;
        } catch (Exception ignored) {
            return null;

...
import tkm.tmnote.pro.adapters.NotesAdapter;
...
public class MainActivity extends AppCompatActivity {
    ...
    private TextView countAll, countPinned, countFav, countCheck, countReminders, countAttachments, countArch, countTrash;
    ...
    private long countsLastUpdated = 0;
    private static final long COUNTS_CACHE_TTL_MS = 5000L;
    ...
    protected void onCreate(Bundle b) {
        ...
        countAttachments = findViewById(R.id.count_attachments);
        ...
        btnSearch.setOnClickListener(v -> {
            ...
            i.putExtra("search_category_id", currentCategoryId);
            i.putExtra("search_filter", currentFilter);
            startActivity(i);
    ...
    /**
     * DrawerLayout плохо дружит с простым padding на корневом view в edge-to-edge
     * режиме Android 15+ — из-за этого в MainActivity toolbar и FAB могут частично
     * уходить под status/navigation bar. Поэтому для главного экрана раскладываем
     * insets точечно: верхние панели получают status-bar отступ, FAB и drawer —
     * navigation/status отступы.
     */
    private void setupMainActivityInsets() {
        if (drawer == null) return;

        final View drawerContent = findViewById(R.id.drawer_content);

        final int topBaseLeft = topBar != null ? topBar.getPaddingLeft() : 0;
        final int topBaseTop = topBar != null ? topBar.getPaddingTop() : 0;
        final int topBaseRight = topBar != null ? topBar.getPaddingRight() : 0;
        final int topBaseBottom = topBar != null ? topBar.getPaddingBottom() : 0;

        final int selBaseLeft = selectionBar != null ? selectionBar.getPaddingLeft() : 0;
        final int selBaseTop = selectionBar != null ? selectionBar.getPaddingTop() : 0;
        final int selBaseRight = selectionBar != null ? selectionBar.getPaddingRight() : 0;
        final int selBaseBottom = selectionBar != null ? selectionBar.getPaddingBottom() : 0;

        final int drawerBaseLeft = drawerContent != null ? drawerContent.getPaddingLeft() : 0;
        final int drawerBaseTop = drawerContent != null ? drawerContent.getPaddingTop() : 0;
        final int drawerBaseRight = drawerContent != null ? drawerContent.getPaddingRight() : 0;
        final int drawerBaseBottom = drawerContent != null ? drawerContent.getPaddingBottom() : 0;

        final int recyclerBaseLeft = recycler != null ? recycler.getPaddingLeft() : 0;
        final int recyclerBaseTop = recycler != null ? recycler.getPaddingTop() : 0;
        final int recyclerBaseRight = recycler != null ? recycler.getPaddingRight() : 0;
        final int recyclerBaseBottom = recycler != null ? recycler.getPaddingBottom() : 0;

        final View fabView = fab;
        final android.view.ViewGroup.MarginLayoutParams fabLp =
                fabView != null && fabView.getLayoutParams() instanceof android.view.ViewGroup.MarginLayoutParams
                        ? (android.view.ViewGroup.MarginLayoutParams) fabView.getLayoutParams()
                        : null;
        final int fabBaseBottomMargin = fabLp != null ? fabLp.bottomMargin : 0;
        final int fabBaseLeftMargin = fabLp != null ? fabLp.leftMargin : 0;
        final int fabBaseTopMargin = fabLp != null ? fabLp.topMargin : 0;
        final int fabBaseRightMargin = fabLp != null ? fabLp.rightMargin : 0;
        ...
        androidx.core.view.ViewCompat.setOnApplyWindowInsetsListener(drawer, (v, insets) -> {
            ...
            if (topBar != null) {
                topBar.setPadding(topBaseLeft, topBaseTop + bars.top, topBaseRight, topBaseBottom);
            }
            if (selectionBar != null) {
                selectionBar.setPadding(selBaseLeft, selBaseTop + bars.top, selBaseRight, selBaseBottom);
            }
            if (drawerContent != null) {
                drawerContent.setPadding(
                        drawerBaseLeft,
                        drawerBaseTop + bars.top,
                        drawerBaseRight,
                        drawerBaseBottom + bars.bottom
                );
            }
            if (recycler != null) {
                recycler.setPadding(
                        ...
                        recyclerBaseTop,
                        recyclerBaseRight,
                        recyclerBaseBottom + bars.bottom
                );
            }
            if (fabLp != null && fabView != null) {
                fabLp.setMargins(
                        fabBaseLeftMargin,
                        fabBaseTopMargin,
                        fabBaseRightMargin,
                        fabBaseBottomMargin + bars.bottom
                );
    ...
    private void updateScreenTitle() {
        ...
        switch (currentFilter) {
            ...
            case 7: tvScreenTitle.setText(getString(R.string.main_filter_attachments)); break;
    ...
    private void setupDrawerStaticItems() {
        ...
        findViewById(R.id.drawer_attachments).setOnClickListener(v -> selectFilter(-1, 7));
        ...
        findViewById(R.id.drawer_stats).setOnClickListener(v -> {
            HapticUtils.light(v);
            navigateFromDrawer(new Intent(this, SupportActivity.class));
        });
    ...
    private void applyDrawerMenuVisibility() {
        ...
        View vAttachments = findViewById(R.id.drawer_attachments);
        ...
        boolean currentHidden =
             ...
             || (currentFilter == 7 && (mask & PrefsManager.MENU_ITEM_ATTACHMENTS) == 0);
        if (currentHidden) {
            currentCategoryId = -1;
            currentFilter = 0;
        }
    ...
    private void applyActiveDrawerHighlight() {
        ...
        if (systemActive) {
            switch (currentFilter) {
                ...
                case 7: activeId = R.id.drawer_attachments; break;
                default: activeId = R.id.drawer_all;      break;
            }
        }
        int[] allRows = new int[] {
                R.id.drawer_all, R.id.drawer_fav, R.id.drawer_pinned,
                R.id.drawer_reminders, R.id.drawer_check, R.id.drawer_attachments,
    ...
    private void selectFilter(long catId, int filter) {
        ...
        if (drawer != null && drawer.isDrawerOpen(GravityCompat.START)) {
            ...
            Runnable fallback = applyFilterAction;
            fallbackHolder[0] = fallback;
            drawer.postDelayed(fallback, 350L);
        } else {
            applyFilterAction.run();
        }
    }
    ...
    private void updateDrawerCounts() {
        ...
        AppExecutors.getInstance().diskIO().execute(() -> {
            ...
            AppExecutors.getInstance().mainThread(() -> {
                ...
                cachedCounts = counts;
                countsLastUpdated = System.currentTimeMillis();
                applyCounts(counts);
            });
        });
    }
    ...
    private void applyEmptyState(long categoryFilter, int filter) {
        ...
        switch (filter) {
            ...
            case 6:
                ...
                break;
            case 7:
                emptyDesc.setText(R.string.empty_attachments_desc);
                break;
            default:
    ...
    private void applyFilterUiRules() {
        ...
        if (btnPin != null) {
            btnPin.setVisibility(inTrash ? View.GONE : View.VISIBLE);
        }
        if (btnColor != null) {
            btnColor.setVisibility(inTrash ? View.GONE : View.VISIBLE);
        }
        if (btnCategory != null) {
            btnCategory.setVisibility(inTrash ? View.GONE : View.VISIBLE);
        }
    ...
    private void setupSelectionBar() {
        ...
        findViewById(R.id.sel_pin).setOnClickListener(v -> {
            ...
            adapter.clearSelection();
            snack(getString(shouldUnpin ? R.string.main_selection_unpinned : R.string.main_selection_pinned, unlocked.size()), Snackbar.LENGTH_SHORT).show();
            loadNotes();
        });
    ...
    private void applyCategoryToSelection(long catId, BottomSheetDialog sheet, List<Note> selectedNotes) {
        ...
        adapter.clearSelection();
        Snackbar.make(recycler,
                catId == 0 ? getString(R.string.main_category_cleared) : getString(R.string.main_category_updated), Snackbar.LENGTH_SHORT).show();
        loadNotes();
    }

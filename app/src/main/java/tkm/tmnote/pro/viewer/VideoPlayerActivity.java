...
public class VideoPlayerActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    private ImageView btnPlay, btnPrev, btnNext, btnRew, btnFf, btnRotate, btnPlaylist;
    ...
    private void showPlaylistDialog() {
        ...
        View view = getLayoutInflater().inflate(R.layout.dialog_video_playlist, null);
        ...
        dialog.setOnShowListener(dialogInterface -> {
            ...
            if (bottomSheet != null) {
                ...
                // Автоматическое раскрытие: если видео больше 6 (то есть больше 2 полных рядов),
                // полностью разворачиваем шторку! Иначе она сама сожмется под размер видеозаписей.
                if (videoAttachments.size() > 6) {
        ...
        PlaylistGridAdapter adapter = new PlaylistGridAdapter(this, videoAttachments, position -> {
            dialog.dismiss();
            loadAndPlayVideo(position);
        });
        ...
        btnDeleteSelected.setOnClickListener(v -> {
            ...
            tkm.tmnote.pro.utils.ConfirmSheet.show(this, "Удалить файлы?", 
                "Выбранные видеозаписи будут полностью удалены из заметки и памяти приложения.", 
                "Удалить", true, () -> {
                    ...
                    for (int index : selectedIndices) {
                        Attachment a = videoAttachments.get(index);
                    ...
                    for (String fileName : filesToDelete) {
                        File f = AttachmentUtils.getFile(VideoPlayerActivity.this, fileName);
                        if (f.exists()) f.delete();
                    }
                    
                    // 2. Удаляем из списка
                    for (Attachment a : attachmentsToRemove) {
                        videoAttachments.remove(a);
                    }
                    ...
                    if (videoAttachments.isEmpty()) {
                        ...
                        if (currentPosition >= videoAttachments.size()) {
                            currentPosition = videoAttachments.size() - 1;
                        }
                        loadAndPlayVideo(currentPosition);
                        dialog.dismiss();
                    }
    ...
    private class PlaylistGridAdapter extends RecyclerView.Adapter<PlaylistGridAdapter.VH> {
        ...
        public void onBindViewHolder(@NonNull VH h, int pos) {
            ...
            h.ivPlayBadge.setVisibility(View.VISIBLE);

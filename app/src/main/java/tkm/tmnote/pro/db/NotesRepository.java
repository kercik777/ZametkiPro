...
public class NotesRepository {
    ...
    public List<Note> searchNotesContextual(String query, long categoryFilter, int filter) {
        return query(categoryFilter, filter, query, PrefsManager.SORT_DATE_DESC);
    }
    ...
    private List<Note> query(long categoryFilter, int filter, String searchQuery, int sortMode) {
        ...
        try {
            ...
            else if (filter == 7) where.append(" AND ").append(DbHelper.C_ATTACHMENTS).append(" IS NOT NULL AND ")
                    .append(DbHelper.C_ATTACHMENTS).append(" != '[]' AND ")
                    .append(DbHelper.C_ATTACHMENTS).append(" != ''");
    ...
    public DashboardCounts getDashboardCounts() {
        ...
        try {
            ...
            String sql = "SELECT " +
                    ...
                    "SUM(CASE WHEN " + DbHelper.C_PINNED + "=1 AND " + visibleCond + " THEN 1 ELSE 0 END), " +
                    "SUM(CASE WHEN " + DbHelper.C_FAVORITE + "=1 AND " + visibleCond + " THEN 1 ELSE 0 END), " +
                    "SUM(CASE WHEN " + DbHelper.C_TYPE + "=" + Note.TYPE_CHECKLIST + " AND " + visibleCond + " THEN 1 ELSE 0 END), " +
                    "SUM(CASE WHEN " + DbHelper.C_REMINDER + ">0 AND " + visibleCond + " THEN 1 ELSE 0 END), " +
    ...
    /**
     * Удаляет ВСЕ заметки и ВСЕ категории безвозвратно.

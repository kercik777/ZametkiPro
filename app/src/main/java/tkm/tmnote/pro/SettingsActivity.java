...
import tkm.tmnote.pro.utils.AppRestartHelper;
import tkm.tmnote.pro.utils.AttachmentUtils;
import tkm.tmnote.pro.utils.BiometricHelper;
...
public class SettingsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle b) {
        ...
        previewLinesRow.setOnClickListener(v -> {
            HapticUtils.light(v);
            showRangeSheet(previewLinesValue, R.string.settings_preview_lines, 1, 50,
                    prefs.getPreviewMaxLines(), PrefsManager.DEFAULT_PREVIEW_MAX_LINES, prefs::setPreviewMaxLines);
        });
        ...
        attCountRow.setOnClickListener(v -> {
            HapticUtils.light(v);
            showRangeSheet(attCountValue, R.string.settings_attachment_columns, 1, 5,
                    prefs.getAttachmentColumns(), PrefsManager.DEFAULT_ATTACHMENT_COLUMNS, prefs::setAttachmentColumns);
        });
        ...
        View checklistRow = findViewById(R.id.row_checklist_completed);
        checklistRow.setOnClickListener(v -> {
            HapticUtils.light(v);
            showChecklistActionSheet();
        });
    ...
    private void showDefaultTextSizeSheet(TextView valueLabel) {
        ...
        final int defaultValue = PrefsManager.DEFAULT_TEXT_SIZE;
        final int initial = prefs.getDefaultTextSize();

        tvTitle.setText(R.string.settings_default_text_size);
        ...
        btnReset.setOnClickListener(view -> {
            HapticUtils.light(view);
            prefs.setDefaultTextSize(defaultValue);
            valueLabel.setText(String.valueOf(defaultValue));
            sheet.dismiss();
        });
    ...
    private void showChecklistActionSheet() {
        BottomSheetDialog sheet = new BottomSheetDialog(this);
        View v = LayoutInflater.from(this).inflate(R.layout.sheet_checklist_action, null);
        sheet.setContentView(v);

        RadioGroup rg = v.findViewById(R.id.rg_checklist_action);
        RadioButton rbMoveToBottom = v.findViewById(R.id.rb_move_to_bottom);
        RadioButton rbLeavePlace   = v.findViewById(R.id.rb_leave_place);
        RadioButton rbDelete       = v.findViewById(R.id.rb_delete);

        int current = prefs.getChecklistAction();
        switch (current) {
            case PrefsManager.CHECKLIST_ACTION_LEAVE_PLACE: rbLeavePlace.setChecked(true); break;
            case PrefsManager.CHECKLIST_ACTION_DELETE:      rbDelete.setChecked(true); break;
            case PrefsManager.CHECKLIST_ACTION_MOVE_TO_BOTTOM:
            default:                                        rbMoveToBottom.setChecked(true); break;
        }

        rg.setOnCheckedChangeListener((group, checkedId) -> {
            int action;
            if (checkedId == R.id.rb_leave_place) action = PrefsManager.CHECKLIST_ACTION_LEAVE_PLACE;
            else if (checkedId == R.id.rb_delete) action = PrefsManager.CHECKLIST_ACTION_DELETE;
            else action = PrefsManager.CHECKLIST_ACTION_MOVE_TO_BOTTOM;
            
            HapticUtils.light(group);
            prefs.setChecklistAction(action);
            group.postDelayed(() -> { if (sheet.isShowing()) sheet.dismiss(); }, 180L);
        });
    ...
    private void showMenuItemsSheet() {
        ...
        CheckBox cbAttachments = v.findViewById(R.id.cb_attachments);
        ...
        cbAttachments.setChecked((initial & PrefsManager.MENU_ITEM_ATTACHMENTS) != 0);
        ...
        v.findViewById(R.id.btn_done).setOnClickListener(click -> {
            ...
            if (cbAttachments.isChecked()) mask |= PrefsManager.MENU_ITEM_ATTACHMENTS;
    ...
    private void showRangeSheet(TextView valueLabel, int titleRes, int min, int max, int initial,
                                int defaultValue, java.util.function.IntConsumer onApply) {
        BottomSheetDialog sheet = new BottomSheetDialog(this);
        View v = LayoutInflater.from(this).inflate(R.layout.sheet_text_size, null);
        sheet.setContentView(v);
        ...
        final int fMin = min;
        final int fDefaultValue = defaultValue;
        ...
        btnReset.setOnClickListener(view -> {
            HapticUtils.light(view);
            onApply.accept(fDefaultValue);
            valueLabel.setText(String.valueOf(fDefaultValue));
            sheet.dismiss();
        });
        btnCancel.setOnClickListener(view -> {
            HapticUtils.light(view);
